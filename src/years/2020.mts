 ...
 * ES2020 Year Module
 *
 * Exports the jsonv API configured for ES2020 features (2015 base + 2020):
 * - All ES2015 features
 * - BigInt literals with 'n' suffix (123n)
 * - BigInt in all number bases (hex, binary, octal)
 */
...
/**
 * Parse jsonv text with ES2020 features (includes BigInt)
 */
...
function parseWithOptions(text: string, options?: ParseOptions): any {
	return parseWithOptionsImpl(text, { ...options, year: 2020 });
}

/**
 * Stringify value with ES2020 output (BigInt with 'n' suffix)
 */
function stringify(
	value: any,
